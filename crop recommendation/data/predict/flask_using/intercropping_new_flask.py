...
# Paths & Constants
# =============================
# Resolve once so all artifacts use project-relative paths.
DATA_DIR = Path(__file__).resolve().parents[2]
save_dir = DATA_DIR / "trainedmodels" / "new_intern"
model_path = save_dir / "compatibility_model.pth"
soil_encoder_path = save_dir / "soil_encoder.pkl"
crop_encoder_path = save_dir / "crop_encoder.pkl"
scaler_X_path = save_dir / "scaler_X.pkl"
scaler_y_path = save_dir / "scaler_y.pkl"
dataset_json_path = DATA_DIR / "Datasets" / "intercropping" / "intercropping_fixed.json"

# =============================
# Load Dataset & Soil Types
# =============================
...
def preprocess_input(env_numeric, soil_type, crop1, crop2):
    ...
    return (
        ...
        torch.tensor([soil_id], dtype=torch.long).to(device),
        torch.tensor([crop1_id], dtype=torch.long).to(device),
        torch.tensor([crop2_id], dtype=torch.long).to(device),
    )
...
def hybrid_predict_user(env_input, soil_type, crop1, crop2):
    """Returns only Yield Impact % for given crop pair."""
    ...
    with torch.no_grad():
        pred_scaled = model(X_num, X_soil, X_c1, X_c2).cpu().numpy()
    pred = scaler_y.inverse_transform(pred_scaled)
    return float(pred[0][2])  # Yield Impact %

# =============================
# Top Companion Crops Function (like multiheight)
# =============================
def top_companion_crops(user_crop, soil_type, env_input, top_n=5):
    results = []
    for c2 in ALL_CROPS:
        if c2 == user_crop:
            continue
        yi = hybrid_predict_user(env_input, soil_type, user_crop, c2)
        results.append({
            "Crops": [user_crop, c2],
            "Yield_Impact_%": round(yi,2)
        })
    results = sorted(results, key=lambda x: x["Yield_Impact_%"], reverse=True)[:top_n]
    return results
