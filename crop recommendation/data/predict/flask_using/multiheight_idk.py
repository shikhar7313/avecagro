...
# -------------------------
def normalize(text):
    return text.strip().title()  # Capitalizes first letters
...
def load_model_and_scalers():
    ...
    model = SeqMultiHeightNN(dataset, region_count).to(device)
    ...
    return model, x_scaler, y_scaler, mappings, dataset, all_regions, device
...
if __name__ == "__main__":
    ...
    model, x_scaler, y_scaler, mappings, dataset, all_regions, device = load_model_and_scalers()

    # -------- USER INPUTS --------
    user_crop = input("Enter main crop: ").strip()
    soil_type = input("Enter soil type: ").strip()
    region_input = input("Enter region: ").strip()
    season = input("Enter season: ").strip()
    soil_ph = input("Soil pH (or range): ").strip()
    rainfall = input("Rainfall (or range): ").strip()
    temperature = input("Temperature (or range): ").strip()
    humidity = input("Humidity (or range): ").strip()
    ...
    # -------- PREPARE CROPS FOR COMBINATIONS --------
    all_crops_normalized = [normalize(c) for c in dataset.crop_list]
    user_crop_norm = normalize(user_crop)
    remaining_crops = [c for c in dataset.crop_list if normalize(c) != user_crop_norm]

    results = []

    # Generate combinations of size 2 up to max_crops
    ...
    print("\n=== TOP 5 COMPANION CROP COMBINATIONS ===\n")
    for r in results:
        print(f"Crops: {', '.join(r['Crops'])} → Yield Impact: {r['Yield_Impact_%']:.2f}%")

    print(f"\n[INFO] Runtime: {time.time() - start_time:.4f} seconds")
