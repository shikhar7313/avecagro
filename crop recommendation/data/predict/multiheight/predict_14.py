...
class SeqMultiHeightNN(nn.Module):
    def __init__(self, dataset):
        ...
        # ✅ match training: pooled crop embedding → only 1x64, not max_crops*64
    ...
    def forward(self, x_numeric, x_categorical):
        ...
        # ✅ pool crop embeddings instead of flattening
        crop_embs = self.crop_emb(crop_ids)                  # [batch, max_crops, emb_dim]
        mask = (crop_ids != self.pad_idx).unsqueeze(-1).float()
        sum_emb = (crop_embs * mask).sum(dim=1)
        denom = mask.sum(dim=1).clamp(min=1.0)
        crop_e = sum_emb / denom                             # [batch, emb_dim]
