...
class SeqMultiHeightNN(nn.Module):
    def __init__(self, dataset):
        ...
        # ✅ Attention layer for crop embeddings
        self.attn = nn.Linear(CONFIG["embedding_dim"], 1)

        # Input dim = numeric(4) + 4 embeddings (crop pooled + region + season + soil)
    ...
    def forward(self, x_numeric, x_categorical):
        ...
        # Crop embeddings
        crop_embs = self.crop_emb(crop_ids)  # [B, max_crops, emb_dim]
        mask = (crop_ids != self.pad_idx).unsqueeze(-1).float()

        # ✅ Attention pooling
        attn_scores = self.attn(crop_embs).squeeze(-1)              # [B, max_crops]
        attn_scores = attn_scores.masked_fill(mask.squeeze(-1) == 0, -1e9)
        attn_weights = torch.softmax(attn_scores, dim=1).unsqueeze(-1)  # [B, max_crops, 1]
        crop_e = (crop_embs * attn_weights).sum(dim=1)              # [B, emb_dim]
...
def predict(sample_input, dataset, model, x_scaler, y_scaler, mappings, device):
    ...
    soil_ph = pick_from_range(sample_input["Shared_Environment"]["Soil_pH_Range"])
    rainfall = pick_from_range(sample_input["Shared_Environment"]["Rainfall_mm_Range"])
    temp = pick_from_range(sample_input["Shared_Environment"]["Temperature_C_Range"])
    humidity = pick_from_range(sample_input["Shared_Environment"]["Humidity_%_Range"])
